...
const argv = require("yargs")
	...
	.describe("x", "Gap mode")
	.boolean("x")
	.describe("filter-loop", "Call external filter when OTF files is built")
	.string("filter-loop")
	.describe("ttx-loop", "Call TTX in between to get the components optimized")
	.boolean("ttx-loop")
...
async function main() {
	...
	if (argv.o) {
		...
		const buffers = await buildOtf(otdFiles, tempDir, argv["filter-loop"], argv["ttx-loop"]);
...
async function buildOtf(otdPaths, tempDir, filter, fTtxLoop) {
	// build OTF
	let buffers = [];
	for (let pOtd of otdPaths) {
		...
		if (filter) {
			const [filterProgram, ...filterArgs] = filter.split(/ +/g);
			const pOtf1 = temp.path({ dir: tempDir, suffix: ".otf" });
			await spawn(which.sync(filterProgram), [...filterArgs, pOtf, pOtf1]);
			await fs.remove(pOtf);
			pOtf = pOtf1;
		} else if (fTtxLoop) {
			const pTtx = temp.path({ dir: tempDir, suffix: ".ttx" });
			const pOtf1 = temp.path({ dir: tempDir, suffix: ".otf" });
			await spawn(Config.ttxCommand, ["-q", "-o", pTtx, pOtf]);
			await spawn(Config.ttxCommand, ["-q", "-o", pOtf1, pTtx]);
			await fs.remove(pTtx);
			await fs.remove(pOtf);
			pOtf = pOtf1;
		}
