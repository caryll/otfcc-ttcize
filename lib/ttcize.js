...
async function main() {
	...
	if (argv.o) {
		...
		const buffers = await buildOtf(otdFiles, tempDir, argv["ttx-loop"]);
...
async function buildOtf(otdPaths, tempDir, fTtxLoop) {
	// build OTF
	let buffers = [];
	for (let pOtd of otdPaths) {
		...
		if (fTtxLoop) {
			let pTtx = temp.path({ dir: tempDir, suffix: ".ttx" });
			let pOtf1 = temp.path({ dir: tempDir, suffix: ".otf" });
